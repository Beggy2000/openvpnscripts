...
# ----------  end of function createFullArchive  ----------


#===  FUNCTION  ================================================================
#          NAME:  createOVPNArchive
#   DESCRIPTION:  
#    PARAMETERS:
#       RETURNS:
#===============================================================================
function createOVPNArchive () {
    declare -r RELATIVE_CLIENT_OVPN="${CLIENT_OVPN#${OPEN_VPN_DIR}/}"
    declare -r RELATIVE_CLIENT_CRT="${CLIENT_CRT#${OPEN_VPN_DIR}/}"
    declare -r RELATIVE_CLIENT_KEY="${CLIENT_KEY#${OPEN_VPN_DIR}/}"
    declare -r RELATIVE_CA_CRT="${CA_CRT#${OPEN_VPN_DIR}/}"
    declare -r RELATIVE_TSL_SECRET="${TSL_SECRET#${OPEN_VPN_DIR}/}"

    cat - > "${CLIENT_OVPN}" <<END_OF_CLIENT_OVPN
#client option implies pull as well
client
...
END_OF_CLIENT_OVPN
    ...
    tar --auto-compress --create --directory "${OPEN_VPN_DIR}" --file "${CURRENT_DIR}/${CLIENT_NAME}.tar.gz" "${RELATIVE_CLIENT_OVPN}" 
    rm "${CLIENT_OVPN}" 
}
# ----------  end of function createOVPNArchive  ----------
